...
let state={
  name:'',email:'',phone:'',gift:'',network:'',reference:'',registrationSynced:false,
  stage:'landingScreen',shareProgress:0,shareComplete:false,
...
function makeReference(){return `PJ-WW-${Math.floor(10000+Math.random()*90000)}`}
...
function setRegistrationStatus(message,isError=false){const el=$('registrationStatus');if(!el)return;el.textContent=message;el.classList.toggle('show',Boolean(message));el.style.color=isError?'#a12744':'#167a55'}
async function submitRegistration(){
  ...
  const validPhone=/^(?:\+?234|0)[789][01]\d{8}$/.test(phone);
  $('fullNameError').classList.toggle('show',!validName);
  $('emailError').classList.toggle('show',!validEmail);
  $('phoneError').classList.toggle('show',!validPhone);
  if(!validName||!validEmail||!validPhone)return;

  state.name=name;state.email=email;state.phone=phone;state.reference=state.reference||makeReference();save();
  const button=$('registrationProceed');
  const originalText=button.textContent;
  button.disabled=true;button.textContent='SAVING YOUR DETAILS…';setRegistrationStatus('');

  try{
    await window.PellerRegistration.save({
      full_name:name,
      email,
      phone_number:phone,
      selected_gift:state.gift,
      mobile_network:state.network||null,
      registration_reference:state.reference,
      source_page:location.hostname
    });
    state.registrationSynced=true;save();
    setRegistrationStatus('Details saved successfully.');
    runTransition(()=>state.gift==='data'?showScreen('dataScreen'):showScreen('shareScreen'));
  }catch(error){
    state.registrationSynced=false;save();
    console.error('Registration save failed:',error);
    setRegistrationStatus('Could not save online. Check your connection and tap again.',true);
  }finally{
...
async function syncNetwork(){if(state.registrationSynced&&state.reference){try{await window.PellerRegistration.updateNetwork(state.reference,state.network)}catch(error){console.error('Network update failed:',error)}}}
function renderShare(){$('sharePersonalCopy').textContent=`${firstName()}, complete the WhatsApp sharing step to continue with your selected gift.`;$('shareGiftBadge').innerHTML=giftSummary(state.gift,true);sharing.update()}
...
renderNetworks();$('dataProceed').onclick=async()=>{await syncNetwork();runTransition(()=>showScreen('shareScreen'))};
...
if(state.name){$('fullName').value=state.name;$('emailAddress').value=state.email||'';$('phoneNumber').value=state.phone}
